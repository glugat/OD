...
# Fonction pour obtenir le token d'accès pour l'API France Travail
def get_token_emploi(id_api_emploi, cle_api_emploi):
    ...
    if response.status_code == 200:
        ...
        print("❌ Échec de l'authentification :", response.status_code)
        print(response.text)
        return None 


def get_job_offers(city_name, access_token):
    # Trouver le code INSEE de la ville
    code_insee = communes[communes['nom_standard'] == city_name]['code_insee'].values[0]
    
    url = "https://api.pole-emploi.io/partenaire/offresdemploi/v2/offres/search"
    
    # Paramètres de la requête, incluant le code INSEE et la recherche par mots-clés
    params = {
        "commune": code_insee,
        "rayon": 10,         # Rayon de recherche en km
        "range": "0-10",     # Plage de résultats (10 premiers)
        "motsCles": "data",  # Mots-clés pour la recherche d'offres (exemple "data")
    }
    ...
    if response.status_code == 200:
        return response.json().get("content", [])  # Retourne les offres d'emploi
    else:
        print(f"Erreur lors de la récupération des offres d'emploi : {response.status_code}")
        return []
...
communes_original = pd.read_excel("communes-france-2025_v4.xlsx")
        ...
        with ui.nav_panel("Emploi"):
            ...
            with ui.layout_columns():  
                with ui.card():  
                    ...
                    # Fonction pour afficher les offres d'emploi pour la première ville
                    @render.ui
                    def display_job_offers_1():
                        ville_1 = input.selectize_v1()
                        if ville_1:
                            # Récupérer le token d'accès
                            access_token = get_token_emploi(id_api_emploi, cle_api_emploi)
                            if access_token:
                                job_offers = get_job_offers(ville_1, access_token)
                                if job_offers:
                                    content = []
                                    for offer in job_offers:
                                        ...
                                        content.append(
                                            ui.div(
                                                ui.h5(f"Offre : {title}"),
                                                ui.p(f"Entreprise : {company}"),
                                                ui.p(f"Lieu : {location}"),
                                                ui.hr()
                                            )
                                        )
                                    return ui.div(*content)
                                else:
                                    return ui.p("Aucune offre d'emploi trouvée.")
                            return ui.p("Impossible d'obtenir les offres d'emploi.")
                        return ui.p("Sélectionnez une ville pour voir les offres d'emploi.")
